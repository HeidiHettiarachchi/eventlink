...
// let staffAdvisorRoutes = express.Router();

// // Route to check and retrieve staff advisor details for an event
// staffAdvisorRoutes.route("/staff-advisor/:eventId").get(verifyToken, async (request, response) => {
//     let db = database.getDb();
...
//         response.status(200).json(staffAdvisor);
//     } catch (error) {
//         console.error("Error fetching staff advisor:", error);
//         response.status(500).json({ message: "Internal Server Error" });
//     }
// });
...
//         if (error) {
//             return response.status(403).json({ message: "Authentication Failed: Invalid token." });
//         }
